...
class CreateFeatureView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not created:
            return JsonResponse({
                "error": f'Wartość "{value}" już istnieje'
            })
        messages.add_message(
            request, messages.SUCCESS,
            f'Wartość "{wartość}" dla cechy'
            f'"{feature.feature_name}" w kategorii {category.name} utworzono pomyślnie'
        )
...
class UpdateProductFeaturesAjaxView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.add_message(
            request, messages.SUCCESS,
            f'Wartości funkcji dla produktu {product.title} zostały pomyślnie zaktualizowane'
        )
        return JsonResponse({"result": "ok"})
